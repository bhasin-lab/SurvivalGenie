      ...
      dashboardBody(
        ...
        tabItems(
          ...
          tabItem(tabName="surv_tils", 
				...
				downloadButton(outputId = "downloadTILS",
						   label = 'Download PDF')
                  )),
          ...
          tabItem(tabName="surv_sigs", 
				...
				downloadButton(outputId = "downloadSIG",
						   label = 'Download PDF')
                  )),
          tabItem(tabName="surv_tmb", 
				...
				downloadButton(outputId = "downloadTMB",
						   label = 'Download PDF')
                  )),
          ...
          tabItem(tabName="surv_gep", 
				...
				downloadButton(outputId = "downloadGEP",
						   label = 'Download PDF')
                  )),
          tabItem(tabName="surv_gene", 
				...
				downloadButton(outputId = "downloadGENE",
						   label = 'Download PDF')
                  )),
          tabItem(tabName="surv_ratio", 
				...
				downloadButton(outputId = "downloadRATIO",
						   label = 'Download PDF')
                  )),
          tabItem(tabName="surv_cls", 
				...
				downloadButton(outputId = "downloadCLS",
						   label = 'Download PDF')
                  ))
  ...
  server = function(input, output, session) {
      ...
      showModal(
        modalDialog(title = 
                    ...
                    tags$span(h3("Please post your questions and issues",
                                 tags$a(href ="https://github.com/bhasin-lab/SurvivalGenie", "here", target="_blank"))),
                    easyClose = TRUE,
                    size = "l",
                    footer = modalButton(h3("OK"))
        ))
    ...
    observeEvent(input$run_sigs, {
      ...
      if(length(input$checkdataset) == 0){
        ...
        if(update$checktumor == "primary" & FALSE %in% otag){
          if(update$checkdataset %in% getDisSummary()$primary | update$checkdataset %in% getDisSummary()$other_tag){
            update$run_sigs = NULL
            shinyalert('Not enough samples!, Select another data or tumor type', type = "error")
            #session$sendCustomMessage(type = 'testmessage', message = 'Not enough samples! Select another data or tumor type')
        ...
        if(update$checktumor == "recurrent" & FALSE %in% otag){
          if(update$checkdataset %in% getDisSummary()$recurrent | update$checkdataset %in% getDisSummary()$other_tag){
            update$run_sigs = NULL
            shinyalert('Not enough samples!, Select another data or tumor type', type = "error")
            #session$sendCustomMessage(type = 'testmessage', message = 'Not enough samples! Select another data or tumor type')
        ...
        if(update$checktumor == "metastatic" & FALSE %in% otag){
          if(update$checkdataset %in% getDisSummary()$metastatic | update$checkdataset %in% getDisSummary()$other_tag){
            update$run_sigs = NULL
            shinyalert('Not enough samples!, Select another data or tumor type', type = "error")
            #session$sendCustomMessage(type = 'testmessage', message = 'Not enough samples! Select another data or tumor type')
    ...
    output$clusterSelector <- renderUI({
      ...
      if(update$clsfiletype == "URL"){
        dataFile <- input$file_url
        sig_list <-  read.csv(dataFile)
      } 
      if(update$clsfiletype == "ED"){
    ...
    os_pval_CLS_obj <- function(){
      ...
      legend("topright", c(paste0("Low (n=", length(los), ")"), paste0("High (n=", length(his), ")")), title = input$checkdataset, lwd = 4, col = cols, bty = "n", cex = 2.0, border = NA, seg.len = 0.5, x.intersp = 0.6)
      legend("bottomleft", paste0("Logrank p=", signif(summary(cox.os)$sctest["pvalue"], digits=2)),bty = "n", cex = 2.0, border = NA, seg.len = 0.5, x.intersp = 0.5)
	...
	output$hr_scp_GENE <- renderPlotly({
		bp <- hr_scp_GENE_obj()
            ggplotly(bp, tooltip=c("HR"))
	})


    CORR_plot_GENE_obj <- function(){
...
output$downloadGENE <- downloadHandler(
  filename = function() {paste('Survival_Genie_', 'Gene_', Sys.time(), ".pdf", sep="")},
    content = function(file) {
     pdf(file, height=8, width=14)
     print(GENE_plot_obj())
     print(CORR_plot_GENE_obj())
     os_pval_GENE_obj()
     print(hr_scp_GENE_obj())
     print(hrplot_GENE_obj())
     dev.off()
    }
)

output$downloadRATIO<- downloadHandler(
  filename = function() {paste('Survival_Genie_', 'Gene_Ratio_', Sys.time(), ".pdf", sep="")},
    content = function(file) {
     pdf(file, height=8, width=14)
     print(RATIO_plot_obj())
     print(CORR_plot_RATIO_obj())
     os_pval_RATIO_obj()
     print(hr_scp_RATIO_obj())
     print(hrplot_RATIO_obj())
     dev.off()
    }
)

output$downloadSIG <- downloadHandler(
  filename = function() {paste('Survival_Genie_', 'Gene_Sets_', Sys.time(), ".pdf", sep="")},
    content = function(file) {
     pdf(file, height=8, width=14)
     print(wt_plot_obj())
     print(CORR_plot_obj())
     os_pval_SIG_obj()
     print(hr_scp_SIG_obj())
     print(hrplot_SIG_obj())
     dev.off()
    }
)

output$downloadCLS<- downloadHandler(
  filename = function() {paste('Survival_Genie_', 'scCluster_', Sys.time(), ".pdf", sep="")},
    content = function(file) {
     pdf(file, height=8, width=14)
     print(CLS_plot_obj())
     print(CORR_plot_CLS_obj())
     os_pval_CLS_obj()
     print(hr_scp_CLS_obj())
     print(hrplot_CLS_obj())
     dev.off()
    }
)

output$downloadTILS <- downloadHandler(
  filename = function() {paste('Survival_Genie_', 'Cibersoft_TILs_', Sys.time(), ".pdf", sep="")},
    content = function(file) {
...
output$downloadTMB <- downloadHandler(
  filename = function() {paste('Survival_Genie_', 'TMB_', Sys.time(), ".pdf", sep="")},
    content = function(file) {
     pdf(file,height=8, width=14)
     print(tmb_plot_obj())
     print(CORR_plot_TMB_obj())
     os_pval_TMB_obj()
     print(hr_scp_TMB_obj())
     print(hrplot_TMB_obj())
     dev.off()
    }
)

output$downloadMAP <- downloadHandler(
  filename = function() {paste('Survival_Genie_', 'Digital_TILs_', Sys.time(), ".pdf", sep="")},
    content = function(file) {
     pdf(file, height=8, width=14)
     print(map_plot_obj())
     print(CORR_plot_MAP_obj())
     os_pval_MAP_obj()
     print(hr_scp_MAP_obj())
     print(hrplot_MAP_obj())
     dev.off()
    }
)

output$downloadGEP <- downloadHandler(
  filename = function() {paste('Survival_Genie_', 'GEP_', Sys.time(), ".pdf", sep="")},
    content = function(file) {
     pdf(file, height=8, width=14)
     print(gep_plot_obj())
     print(CORR_plot_GEP_obj())
     os_pval_GEP_obj()
     print(hr_scp_GEP_obj())
     print(hrplot_GEP_obj())
     dev.off()
    }
)
